...
void Sensor::body() {
   // std::cout<<type<<" has current level equal to: "<<battery.getCurrentLevel()<<std::endl;
    if (!isActive() && battery.getCurrentLevel() > 90){
        //data=-99999;
        //transfer(data);
        //uint64_t ms = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
//std::cout << "MilliSeconds Since Epoch: "<< ms << '\n';
        std::cout<<type<<" has been ACTIVATED."<<std::endl;  
        turnOn();
        ...
        //uint64_t ms = duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count();
//std::cout << "MilliSeconds Since Epoch: "<< ms << '\n';
        std::cout<<type<<" has been DEACTIVATED."<<std::endl;     
        turnOff();      
    }
...
void Sensor::turnOn() {
    active = true;
    activate(); //SAMIRA ADDED
}

void Sensor::turnOff() {
    active = false;
    deactivate(); //SAMIRA ADDED
}
