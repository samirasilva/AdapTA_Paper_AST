...
void G4T1::process(){
    ...
    std::vector<double> raw_current_data;
    ...
    double diff_s=0;
    if(!started){ //beginning of interval
        initial_timestamp=ms;
        started=true;
    }else{
        uint64_t diff_ms = ms-initial_timestamp;
        diff_s = diff_ms*0.001;
    }
    
    if(testing){
        std::cout<<"$$$$$$$$$$$$$$$$$$$$$$$$ Testing... Diff: "<<diff_s<<std::endl;
        if(diff_s>=testing_time){
            testing=false;
            started=false;
        }else{
            //Testing Time

            if(patient_status > 66.0){
                std::cout << "============ EMERGENCY ============(" << patient_status << '%' << ")" << std::endl;
            }
            else{
                std::cout << "General risk status: " << patient_status << '%' << std::endl;
            }
        ...
        std::cout<<"$$$$$$$$$$$$$$$$$$$$$$$$ Pause... Diff: "<<diff_s<<std::endl;
        if(diff_s>=timebreak){
            testing=true;
            started=false;
        }
...
/*     int time_slot;
    ...
    if (initial_timestamp==0){
        ...
        uint64_t diff_ms = ms-initial_timestamp;
        //std::cout<<"diff_ms: "<<diff_ms<<std::endl;
        double diff_s = diff_ms*0.001;
        //std::cout<<"diff_s: "<<diff_s<<std::endl;
        time_slot = std::ceil(diff_s/float(300));
    }

    int rest =  (time_slot-1) % 4;
    //std::cout<<"rest: "<<rest<<std::endl;
    std::cout<<"$$$$$$$$$$$$$$$$$$$$$$$ --- Slot: "<<std::to_string(time_slot)<<std::endl;
 */
    // if(rest==0){
    // }else{
    //         //Pause Time 
